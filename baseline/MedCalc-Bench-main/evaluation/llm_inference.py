    ...
    def __init__(self, llm_name="OpenAI/gpt-3.5-turbo", cache_dir="../../huggingface/hub"):
        ...
        if self.llm_name.split('/')[0].lower() == "openai":
            ...
            if "gpt-3.5" in self.model or "gpt-35" in self.model:
                ...
                self.max_length = 8192  # 为qwen2设置合适的最大长度
    ...
    def generate(self, messages, prompt=None):
        ...
        if "openai" in self.llm_name.lower():
            try:
                ...
                if "inappropriate content" in str(e).lower():
                    print(f"跳过当前数据项: 内容被判定为不适当 - {str(e)}")
                    ans = "ERROR_INAPPROPRIATE_CONTENT"
                else:
                    print(f"OpenAI API 错误: {str(e)}")
                    ans = f"ERROR_OPENAI_API: {str(e)}"
            except Exception as e:
                print(f"未知错误: {str(e)}")
                ans = f"ERROR_UNKNOWN: {str(e)}"
